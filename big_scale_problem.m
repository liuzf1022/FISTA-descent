...
% 调用FISTA_descent方法
[K, x_min, f_min, X, F, dF_norm] = FISTA_descent_method(ones(n,1), 1e-3,A,b,0.33,m,n);
disp(['利用FISTA_descent方法: 迭代次数 = ', num2str(K), ', 最优值点 x1 = ', num2str(x_min(1)), ', x2 = ', num2str(x_min(2)), ', 最小值 = ', num2str(f_min)]);

% 调用梯度下降法
[K1, x_min1, f_min1, X1, F1, dF_norm1] = DG_method(ones(n,1), 1e-3,A,b,m,n);
...
% FISTA_descent 方法
function [K, x_min, f_min, X, F, dF_norm] = FISTA_descent_method(start, eps,A,b,rho,m,n)
    ...
    while dF_norm(k) >= eps && k < max_iter
        ...
        [alpha,U,r] = line_search_2(alpha,rho,A,b,alpha_0,r,v,X(:, k));
        
        if f(U,A,b) <= f(X(:, k),A,b)
            X(:,k+1) = U;
        else
            X(:,k+1) = X(:,k);
        end
